...
def gen_sineembed_for_position(pos_tensor, d_model=256):   #FOR QUERY 使用
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=300, num_encoder_layers=6,
                 ...
                 return_intermediate_dec=False, query_dim=4,#xywh的意思
                 keep_query_pos=False, query_scale_type='cond_elewise',
                 num_patterns=0,
                 modulate_hw_attn=True,
                 bbox_embed_diff_each_layer=False, #每個decoder是否使用不同的bbox頭
                 ):
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed):
        ...
        bs, c, h, w = src.shape   # h:23 c:256 bs:3 w:32
        #[hw,bs,256]
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            #經過一個mlp (最下面那個)
            # [300,bs,256]
            query_pos = self.ref_point_head(query_sine_embed)   #ref_point_head是一個MLP 
            ...
            #與conditional detr same
            #結構圖中第二行的mlp 左邊的那格mlp
            # apply transformation
            query_sine_embed = query_sine_embed[...,:self.d_model] * pos_transformation

            # modulated HW attentions
            if self.modulate_hw_attn:
                #結構圖中第二行的mlp 右邊的那格mlp
                #公式7的wref href [300,bs,2]
                
                refHW_cond = self.ref_anchor_head(output).sigmoid() # nq, bs, 2
                #公式6的xref
                query_sine_embed[..., self.d_model // 2:] *= (refHW_cond[..., 0] / obj_center[..., 2]).unsqueeze(-1) #0:x 2:w
                #公式6的yref
                query_sine_embed[..., :self.d_model // 2] *= (refHW_cond[..., 1] / obj_center[..., 3]).unsqueeze(-1) #1:y 3:h
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 rm_self_attn_decoder=False): #self attention不使用了
        super().__init__()
        
        #跟CONDITIONAL DETR是一樣的 只是多了兩個參數
        #
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            ...
            #這部分相比DETR就是多了這些LINEAR 多了一層 其他都是一樣的
            q_content = self.sa_qcontent_proj(tgt)      # target is the input of the first decoder layer. zero by default.
            #query position在self attention使用
        ...
        #如果繼續使用query_pos 那麼query_pos也會加入倒cross-attention的query中 否則不會使用
        #cross-attention 正常使用的那個經過公式6處理的那種
        # For the first decoder layer, we concatenate the positional embedding predicted from 
        # the object query (the positional embedding) into the original query (key) in DETR.
        if is_first or self.keep_query_pos:
            #只有第一層decoder會對query_pos進行使用 後面就不會使用了
