...
class DABDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        # default pipeline [300,4], 300個查詢 每個是xywh
        embedweight = self.refpoint_embed.weight
        
        #hs [6,bs,300,256] decoder之後的輸出
        #reference [6,bs,300,4] 為參考點
        hs, reference = self.transformer(self.input_proj(src), mask, embedweight, pos[-1])
        
        
        #bbox_embed_diff_each_layer=False  如果6層decoder共享了一個bbox頭 則輸出傳到self.bbox_embed即可
        #網路預測是相對於參考點座標的偏移量 所以相加即可
