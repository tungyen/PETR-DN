...
class DABDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        # 圖上class label embedding and indicator
        self.label_enc = nn.Embedding(num_classes + 1, hidden_dim - 1) #  +1是因為多了unknown, -1是因為最後一位是indicator
        ...
        assert query_dim in [2, 4]    # 2 for point, 4 for box
        ...
        if random_refpoints_xy:
            ...
            #sigmoid反函數 得到特徵圖上的座標形式
            self.refpoint_embed.weight.data[:, :2] = inverse_sigmoid(self.refpoint_embed.weight.data[:, :2])
            #xy不再更新 xy不會跟隨網路結構更新 但是會在具體的 經歷了各個decoder之後 還是會逐層更新的
    ...
    def forward(self, samples: NestedTensor, dn_args=None):
        ...
        """
        dn_args的內容 targets, args.scalar, args.label_noise_scale, args.box_noise_scale, args.num_patterns

        targets                     GT的信息(座標, label, id....)
        args.scalar                 number of dn groups   ex:(5組)
        args.label_noise_scale      label noise ratio to flip，控制多少比例的label做隨意的更改  ex:(0.2) 
        ...
        #prepare_for_dn 這裡是去躁大重點 /dn_components.py
        ...
        # 多了一個mask_dict的返回值
        # mask_dict 用於計算去躁部分的loss 上面兩個output是用於正常計算匹配的部分
